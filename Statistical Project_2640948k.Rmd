...
Despite Age and gender, discrete number and nominal data respectively, the remaining variables were grouped by row mean, applying Cronbach's alpha for justifying the grouping process. Both "Frequency of App Usage" and "How often sharing on social media" are ordinal data, anchored with the scale from 1 = "never" to 7 = "very often". Former accessing the extent of players using Pokemon Go per month, while the latter demonstrate the frequency players sharing their achievements on social media. The remaining ordinal variables of data was discussed below.
